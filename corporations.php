...
<head>
  ...
  <title> Corporations Listing </title>
        ...
        function showEMPDetails(mail) {
            $.ajax({
                ...
                success: function(response) {
                    var empDetails = JSON.parse(response);
                    showModal(empDetails, mail);
                },
                error: function() {
                    alert('Error retrieving employer details.');
                }
            });
        }
        function showModal(empDetails, mail) {
          ...
          modalContent += '<h2>Employer Details</h2>';

          for (var key in empDetails) {
            modalContent += '<p><strong>' + key + ': </strong>' + empDetails[key] + '</p>';
          }
      ...
      <div style="width: 100%" class="row">
        <div class="col-md-9">
          <div style=" margin-top: 30px; padding-left: 50px;">
            <h1 id="jbs" style="color:black"><b></b></h1>
            <!-- <form class="example" action="candidates.php">
          ...
          <div class="container" style="padding-left:50px; padding-top:50px; padding-bottom:50px;">
                ...
                foreach ($total as $key => $value) {
                    ...
                    foreach ($value as $k => $v) {
                        if ($k=="email") $tbody .= '<td style="border: 1px solid black; padding: 8px; font-weight: bold; color:white; height: 36px;">
                                        <button onclick="showEMPDetails(\''.$value['email'].'\')"><b> Click </b></button></td>';
